...
export default function TvShowModal({ show, isOpen, onClose }) {
  ...
  useEffect(() => {
    if (isOpen && show) {
      setLoading(true);
      axiosInstance.get(`/tv/${show.id}?api_key=${api_key}`)
        .then(response => {
                ...
                <div className="row">
                  <div className="col-md-4">
                    ...
                    <div className="mt-3">
                      ...
                      <p className="mb-1">
                        <strong>First Air:</strong> {new Date(details.first_air_date).toLocaleDateString()}
                      </p>
                      {details.last_air_date && (
                        <p className="mb-1">
                          <strong>Last Air:</strong> {new Date(details.last_air_date).toLocaleDateString()}
                        </p>
                      )}
                    </div>
                  </div>
